...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self.dim_sh = (self.max_sh_degree + 1) ** 2
        self._xyz = torch.empty(0)
        # TODO: what does this "dc" and "rest" means?
        self._features_dc = torch.empty(0)
        self._features_rest = torch.empty(0)
        self._features_sem = torch.empty(0)
    ...
    def get_features(self):
        ...
        features_sem = self._features_sem
        # this returned feature has shape[-1] = 3
        return torch.cat((features_dc, features_rest, features_sem), dim=1)
    ...
    def get_semantic(self):
        return self._features_sem.reshape([self._features_sem.shape[0], -1])
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        features = torch.zeros(
            (fused_color.shape[0], 3, self.dim_sh)).float().cuda()
        ...
        self._features_rest = nn.Parameter(features[:, :, 1:self.dim_sh].transpose(
            1, 2).contiguous().requires_grad_(True))
    ...
    def init_semantic(self, dataset, scene):
        ...
        semantic = torch.zeros(
            (self.get_xyz.shape[0], scene.train_cameras[1][0].gt_semantic.shape[0])).float().cuda()
        g_cnt = torch.zeros((self.get_xyz.shape[0], 1)).float().cuda()
        xyzs = self.get_xyz
        ...
        with torch.no_grad():
            ...
            for cam in scene.train_cameras[1]:
                ...
                x = (((xy[mask, 0] + 1.0) * h - 1.0) * 0.5).int()
                y = ((-(xy[mask, 1] + 1.0) * w - 1.0) * 0.5).int()
                semantic[mask] += features[:, x, y].permute(1, 0)
                g_cnt[mask] += 1
            semantic /= g_cnt

        self.dim_sem = semantic.shape[1]
        assert self.dim_sem % 3 == 0
        semantic = semantic.reshape([semantic.shape[0], 3, -1])
        self._features_sem = nn.Parameter(
            semantic.transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_sem],
                'lr': training_args.feature_lr / 20.0, "name": "f_sem"},
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._features_sem.shape[1]*self._features_sem.shape[2]):
            l.append('f_sem_{}'.format(i))
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        f_rest = self._features_rest.detach().transpose(
            1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_sem = self._features_sem.detach().transpose(
            1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        ...
        attributes = np.concatenate(
            (xyz, normals, f_dc, f_rest, f_sem, opacities, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        sem_f_names = [
            p.name for p in plydata.elements[0].properties if p.name.startswith("f_sem_")]
        sem_f_names = sorted(sem_f_names, key=lambda x: int(x.split('_')[-1]))
        assert len(sem_f_names) == self.dim_sem
        features_sem = np.zeros((xyz.shape[0], len(sem_f_names)))
        for idx, attr_name in enumerate(sem_f_names):
            features_sem[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P, dim_sem) to (P, 3, dim_sem / 3)
        features_sem = features_sem.reshape((features_extra.shape[0], 3, -1))
        ...
        self._features_sem = nn.Parameter(torch.tensor(
            features_sem, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._features_sem = optimizable_tensors["f_sem"]
        self._opacity = optimizable_tensors["opacity"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_features_sem, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
             "f_dc": new_features_dc,
             "f_rest": new_features_rest,
             "f_sem": new_features_sem,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_features_sem = self._features_sem[selected_pts_mask].repeat(
            N, 1, 1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,
                                   new_features_sem, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_features_sem = self._features_sem[selected_pts_mask]
        new_opacities = self._opacity[selected_pts_mask]
        new_scaling = self._scaling[selected_pts_mask]
        new_rotation = self._rotation[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,
                                   new_features_sem, new_opacities, new_scaling, new_rotation)
